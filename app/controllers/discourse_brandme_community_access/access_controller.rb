...
module DiscourseBrandmeCommunityAccess
  class AccessController < ::ApplicationController
    ...
    def handle_existing_user_purchase(
      payload,
      group,
      user
    )
      if group
        ...
        log_access(
          ...
          status: "success",
          message:
            "User already exists and product has no group mapping",
        )

        mark_processed(payload)

        render_success(
          "User #{user.username} already exists — no group assignment required",
        )
      end
    end
    ...
    def handle_existing_invite(
      ...
      if group
        ...
        log_access(
          ...
          status: "success",
          message:
            "Pending invite already exists and product has no group mapping",
        )

        mark_processed(payload)

        render_success(
          "Pending invite already exists — no group assignment required",
        )
      end
    end
    ...
    def create_invite(
      ...
      if group
        ...
        log_access(
          ...
          status: "success",
          message:
            "Standard invite sent without group assignment",
        )

        mark_processed(payload)

        render_success(
          "Invite sent to #{email} without group assignment",
        )
      end
    end

    def add_group_to_invite(
      invite,
      group
    )
    ...
    def handle_refund(payload, group)
      unless group
        return handle_unmapped_refund(payload)
      end
    ...
    def handle_unmapped_refund(payload)
      log_access(
        ...
        status: "success",
        message:
          "Product has no group mapping — no group entitlement to revoke",
      )

      mark_processed(payload)

      render_success(
        "No group mapping — no group access to revoke",
      )
    end
    ...
    def revoke_pending_invite_group(
      payload,
      group,
      email
    )
      ...
      render_success(
        "Pending invite access revoked for group #{group.name}",
      )
    rescue StandardError => e
      log_access(
        payload: payload,
      ...
      render_error(
        "Failed to revoke invite access: #{e.message}",
        :internal_server_error,
      )
    end
